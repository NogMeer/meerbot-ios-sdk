...
public final class MeerBot {
    ...
    private func recordIdentity(_ token: String?) -> IdentityChange {
        ...
        // тот же `sub` со свежим или подписанным несвежим токеном — связь остаётся; строка
        // анонимна (например, после прошлого выхода) — отставлять нечего, поэтому токены без
        // `sub`, каждый из которых выглядит сменой, не уводят в отставку раз за разом.
        // Остаётся один случай: тот же человек без хэша (первый вход после обновления) с
        // ОТКЛОНЁННЫМ токеном — его строка уйдёт в отставку, а тред вернёт первый же свежий
        // подтверждённый вход.
        flags.markLogout(installationId: installationId)
