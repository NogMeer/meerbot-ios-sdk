...
public final class MeerBot {
    ...
    ///
    /// Свежий токен УЖЕ применённого пользователя (тот же `sub`) ленту не трогает, в том числе
    /// после перезапуска приложения (кто был применён, SDK помнит в виде хэша). Любой другой
    /// токен — другой `sub`, `sub`, который не читается, или первый `identify` на установке,
    /// где SDK ещё не знает, кто применён (новая установка, обновление с 0.2.8), — очищает
    /// ленту на экране, а следующая регистрация отвязывает устройство от прежнего — как при
    /// выходе, даже если токен нового сервер не примет.
    ///
    ...
    private func recordIdentity(_ token: String?) -> IdentityChange {
        ...
        // Выход ставится на КАЖДУЮ смену ключа, включая два случая, которые раньше пропускались:
        //   • `sub` не читается. Такой токен сервер отклоняет, связь не трогает, и без флага
        //     устройство осталось бы за прежним — новый человек читал бы его тред;
        //   • прежнего хэша нет. Установка могла быть связана ещё SDK 0.2.8 (он хэш не писал),
        //     и первый вход другого человека после обновления с отклонённым токеном оставлял
        //     ему чужой тред.
        // Лишним флаг не бывает — это обеспечивает таблица сервера (`device-identity.ts`).
        // Связь того же `sub` ОСТАЁТСЯ, если подпись токена верна, — в том числе у токена с
        // истёкшим `exp`: он доказывает, кто это, и строку в отставку не уводит. В отставку
        // строку уводят только токены, по которым сервер не может назвать человека: неверная
        // подпись, не настроенный на приложении ключ (`not_configured`), непригодные claims,
        // битый токен. Строка анонимна (например, после прошлого выхода) — отставлять нечего,
        // поэтому токены без `sub`, каждый из которых выглядит сменой, не уводят в отставку
        // раз за разом. Остаётся один случай: тот же человек без хэша (первый вход после
        // обновления) с ОТКЛОНЁННЫМ токеном — его строка уйдёт в отставку, а тред вернёт
        // первый же принятый вход.
        flags.markLogout(installationId: installationId)
        return .subject(unlinkPrevious: true)
    }
    ...
    /// Что сервер сделал с последним `identityToken`.
    ///
    /// `nil` — только до `configure(...)`. После настройки, пока регистрация не ответила, и
    /// сразу после смены пользователя или выхода — `.notProvided`. Ответ регистрации,
    /// отправленной со старым токеном, статус не меняет.
    ...
    ///
    /// Паритет с Android (`MeerBot.refresh()`). Отличие от `handlePush(_:)`: тот требует
    /// `conversationId` в полезной нагрузке, запоминает его и перечитывает ленту целиком, а
    /// этот догоняет текущую — то есть работает и тогда, когда пуш пришёл без id (у канала он
    /// не обязателен: тред один на устройство, и переключать его ни тот, ни другой не умеет).
    public func refresh() {
        controller?.refresh()
    }

    /// Обработать входящий пуш: запомнить `conversationId` и перечитать ленту треда (тред у
    /// канала один — другой диалог этим не открывается, см. `ChatController.openConversation`).
    ///
    /// Полезная нагрузка: `{"conversationId": 123}` (число или строка).
    ///
    /// - Returns: `true`, если в нагрузке есть `conversationId`, то есть пуш наш. До
    ///   `configure(...)` и посреди смены пользователя пуш при этом ничего не делает, но
    ///   `true` всё равно возвращается: значение говорит, чей пуш, а не что лента обновилась.
    @discardableResult
