...
**Up to Next Minor `0.2.9`** (до 1.0 минорная версия может ломать контракт).
...
`preconnect()`, обновление сессии раз в 15 минут), а не в момент `identify`. Запрашивайте его
прямо перед открытием чата.
...
`not_provided`). Если у приложения включено требование идентификации, анонимная сессия
получит `403 identity_required`.

**Выход (с 0.2.9).** `identify(token: nil)` отвязывает устройство на сервере при следующей
регистрации: прежний тред остаётся за прежним пользователем, новый начинается пустым, лента на
экране очищается сразу. Сигнал сохраняется в `UserDefaults` (в том числе до `configure`) и
снимается только подтверждением сервера. Токен другого `sub` отвязывает прежнего пользователя
и без выхода; свежий токен того же пользователя ленту не трогает; повторный вход того же
пользователя со свежим токеном возвращает его тред. Сервер, не знающий выхода, сигнал
