...
**Статус:** `0.2.9` — рабочий чат на собственном канале платформы (`mobile_app`), один ключ,
verified identity; выход и смена пользователя отвязывают устройство. Не сделано: вложения,
...
> ### ⚠️ Правки — в `agentbot-platform`, не в репозитории `meerbot-ios-sdk`
>
> `github.com/NogMeer/meerbot-ios-sdk` — **зеркало**. Его содержимое целиком
> перезаписывается срезом каталога `mobile-sdk-ios/` из приватного `agentbot-platform`,
> откуда его публикует CI по тегу `ios-X.Y.Z` (ручной аналог — `scripts/release-ios-sdk.sh`).
>
...
**Up to Next Minor `0.2.9`** (до 1.0 минорная версия может ломать контракт). Пакет уже
подключён на `0.2.8` — Xcode держит её в `Package.resolved` и сам не обновит: **File → Packages →
Update to Latest Package Versions** (или `swift package update`).

Или в `Package.swift`:

```swift
dependencies: [
    .package(url: "https://github.com/NogMeer/meerbot-ios-sdk.git", .upToNextMinor(from: "0.2.9"))
]
```
...
> Каждый релиз — тег `ios-X.Y.Z` в `agentbot-platform`: CI (`.github/workflows/ios-sdk-publish.yml`)
> проверяет версию, CHANGELOG, сборку и тесты и публикует срез. Вручную это выглядит так:
> ```bash
> SHA=$(git subtree split --prefix=mobile-sdk-ios HEAD)
> git push --atomic git@github.com:NogMeer/meerbot-ios-sdk.git $SHA:refs/heads/main $SHA:refs/tags/0.2.9
> ```
...
3. Для verified identity — сохранить «Секрет для identity-токена» (тоже показывается один раз;
   потерян — «Перевыпустить секрет identity») и передать разработчику бэкенда.
...
if let controller = MeerBot.shared.chatController() {
    // controller.store — ObservableObject: messages / mode / sending / connectionError / draft
    // controller.send("текст"), controller.retry()
    // черновик — store.draft / store.setDraft(_:), не @State: смена пользователя стирает его с лентой
    // controller.openConversation(id:) — запомнить id из пуша и перечитать ленту текущего треда
}
```
...
func application(
    ...
    // true — в нагрузке есть conversationId, пуш наш. SDK запоминает id и перечитывает ленту
    // текущего треда; до configure и посреди смены пользователя ничего не делает, но true вернёт
    MeerBot.shared.handlePush(payload)
}
```
...
бэкенда (`X-MeerBot-Signature: sha256=HMAC(timestamp.body)` «Секретом подписи вебхука»), а тот
адресует уведомление сам — по `external_user_id`, который вы передали в identity-токене.
Заберите токен из `MeerBot.shared.pushToken` и отправьте на свой сервер вместе с этим же
идентификатором. Контракт вебхука — [meerbot.ru/docs/mobile-sdk/webhook](https://meerbot.ru/docs/mobile-sdk/webhook).
...
Полезная нагрузка вашего пуша должна содержать `conversationId` (число или строка) из вебхука:
`handlePush(payload)` запоминает его и перечитывает ленту текущего треда. Другой диалог так не
открыть — тред у установки один.

### Verified identity

```swift
// перед открытием чата — токен только что получен от вашего бэкенда
MeerBot.shared.identify(token: tokenОтВашегоБэкенда)
// пользователь вышел из аккаунта — и только тогда
MeerBot.shared.identify(token: nil)
```

`token` — HS256-JWT, подписанный **вашим** бэкендом «Секретом для identity-токена» этого
приложения. Claims: `sub` — ваш id пользователя (строка до 255 символов, из авторизации
запроса к вашей ручке), обязательные `iat` и `exp` (не дальше 7 суток), опционально
`email`/`name`. Claim `vid` **не кладите**: сервер сверяет его с `visitorUuid` устройства, а SDK
это значение наружу не отдаёт — любое значение даст `rejected`.

Токен должен быть свежим — не старше 5 минут **в момент регистрации сессии** (показ экрана,
`preconnect()`, первый запрос после `identify`, обновление сессии), а не в момент `identify`.
Запрашивайте его прямо перед открытием чата. 15-минутную сессию SDK обновляет, когда делает
запрос меньше чем за 60 секунд до её истечения, и отправляет последний токен из `identify`. Пока
чат открыт, вызывайте `identify` со свежим токеном не реже раза в 5 минут и при возврате из фона:
сессия, обновлённая со старым токеном, теряет инструменты с доступом к вашим данным (связь и
вебхук остаются). Токен того же `sub` ленту не очищает.

Провал проверки **мягкий**: сессия живёт, пользователь просто анонимен, а причина — в
`await MeerBot.shared.identityStatus()` (`verified` · `stale` · `rejected` · `not_configured` ·
`not_provided`; `nil` — только до `configure`). Если у приложения включено требование
идентификации (включается только на стороне платформы), анонимная сессия получит
`403 identity_required`.

**Выход (с 0.2.9).** `identify(token: nil)` отвязывает устройство на сервере при следующей
регистрации: прежний тред остаётся за прежним пользователем, новый начинается пустым, лента и
черновик на экране очищаются сразу. Сигнал сохраняется в `UserDefaults` (в том числе до
`configure`) и снимается только подтверждением сервера. Любой токен, кроме токена уже
применённого пользователя (другой или нечитаемый `sub`, первый `identify` на установке),
отвязывает прежнего и без выхода; токен того же пользователя ленту не трогает; повторный вход со
свежим токеном, выпущенным после выхода, возвращает тред. Сервер, не знающий выхода, сигнал
игнорирует — там устройство остаётся за последним вошедшим, как в 0.2.8. Вызывайте `nil`
**только на настоящий выход**.
...
| Эскалация | событие `escalation` → `store.mode = .pendingEscalation` |
...
| Идентификация | `identify(token:)` — связь с пользователем вашей системы; `identify(token: nil)` — выход |
